...
def main():
    ...
    **Life-Cycle Cost Analysis for Pavement Alternatives - Version 2.0**
    ...
    with st.sidebar:
        ...
        # ใช้ slider แทน number_input
        พื้นที่ร่วม = st.slider(
            "พื้นที่ (ตร.ม.)",
            min_value=1000.0,
            max_value=100000.0,
            value=10000.0,
            step=1000.0,
            help="พื้นที่โครงการจะใช้ร่วมกันทุกทางเลือก (ปรับที่นี่แล้วจะอัพเดททุกทางเลือก)"
        )
        
        # อัพเดทพื้นที่ทุกทางเลือกอัตโนมัติ (ไม่ต้องกดปุ่ม)
        for ท in st.session_state.ทางเลือกทั้งหมด:
            ท.พื้นที่ = พื้นที่ร่วม
        
        st.caption(f"📏 พื้นที่ปัจจุบัน: {พื้นที่ร่วม:,.0f} ตร.ม.")
    ...
    with tab1:
        ...
        for i, ทางเลือก in enumerate(st.session_state.ทางเลือกทั้งหมด):
            with st.expander(f"{'✅' if ทางเลือก.เปิดใช้งาน else '❌'} ทางเลือกที่ {i+1}: {ทางเลือก.ชื่อ}", expanded=(i==0)):
                ...
                with col5:
                    # แสดงพื้นที่ที่ถูกตั้งจาก Sidebar (ไม่ให้แก้ไข)
                    st.metric(
                        "พื้นที่ (ตร.ม.)",
                        f"{ทางเลือก.พื้นที่:,.0f}",
                        help="ปรับพื้นที่ที่ Sidebar"
                    )
